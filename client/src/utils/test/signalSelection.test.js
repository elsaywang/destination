...
describe('signalSelection Utils', () => {
    ...
    describe('test on hasSignalSelectionsTypeWarning function to validate selected records signal types', () => {
        it('should return true when 1 Onboarded and 1 Real-Time are selected', () => {
            const rowRecords = [
                { signalType: 'Onboarded Records', rowIndex: 4 },
                { signalType: 'Adobe Analytics', rowIndex: 2 },
            ];
            expect(hasSignalSelectionsTypeWarning(rowRecords)).toBe(true);
        });
        ...
        it('should return false when no record is selected', () => {
            const rowRecords = [];
            expect(hasSignalSelectionsTypeWarning(rowRecords)).toBe(false);
        });
